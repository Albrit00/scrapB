...
class PisoscomSpider(CrawlSpider):
    ...
    def parse(self, response):
        ...
        flats = soup.find_all("div", {"class": "ad-preview"})
        
        # Obtenemos si es de alquiler o compra a partir de la URL
        ...
        for nflat in range(len(flats)):
            # Validemos y tomamos el enlace (debe ser el link de la vivienda)
            href = flats[nflat].find(class_="ad-preview__title")['href']
            title = flats[nflat].find(class_="ad-preview__title").text.strip()
        
            # Verificar si se encuentra la clase "ad-preview__logo" dentro de <div class="ad-preview">
            if flats[nflat].find(class_="ad-preview__logo"):
                 # La clase "ad-preview__logo" está presente, continuar con la próxima vivienda
                continue
            else:
                # Buscar en la clase "ad-preview__info" si no se encuentra "ad-preview__logo"
                info_div = flats[nflat].find(class_="ad-preview__info")
                # Resto del código para obtener los datos desde "info_div"
        
                # Intentar obtener municipio, calle y barrio
                town = ''
                neighbour = ''
                street = ''
                number = ''
                # Resto del código para obtener los datos
                ...
                rooms = ''
                rooms_elements = flats[nflat].find_all("p", {"class": "ad-preview__char p-sm"})
                if len(rooms_elements) > 0:
                    rooms = rooms_elements[0].text.strip()
                
                ## Hay pisos sin m2, fecha o planta. Para evitar problemas, asignamos variable vacía si hay error.
                #try:
