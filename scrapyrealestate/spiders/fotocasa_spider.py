    ...
    def parse(self, response):
        ...
        for nflat in range(len(flats)):
            try:
                ...
                if flats[nflat].find("div", {"class":"item-info-container"}).find("div" ,{"class":"logo-branding"}) is not None:
                    # La clase "logo-branding" está presente, continuar con la próxima vivienda
                    continue
                
                # Obtener el enlace de la vivienda
                href = flats[nflat].find("a", {"class": "re-CardPackMinimal-info-container"}, href=True)['href']  # Cambio de div -10/11/2021

            except AttributeError:
                break  # Si no encuentra nada, salimos del bucle
